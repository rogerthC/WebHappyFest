...
(function(){
	...
	function ensureScript(src){
		...
		// shared stylesheet to make header/footer consistent across pages
		const sharedCss = base + '../Organismos/shared-components.css';
		...
		<header class="site-header" id="site-header">
			...
			function insertHeader(html){
				...
				if(placeholder){ placeholder.innerHTML = html; }
					...
					// Normalize injected header: ensure consistent structure and classes
					setTimeout(()=>{
						const hdr = document.getElementById('site-header') || document.querySelector('header.site-header, .site-header');
						if(hdr){
							hdr.classList.add('site-header');
							// ensure inner wrapper exists
							let inner = hdr.querySelector('.header-inner');
							if(!inner){
								// try to wrap existing children in header-inner
								inner = document.createElement('div'); inner.className = 'header-inner';
								while(hdr.firstChild) inner.appendChild(hdr.firstChild);
								hdr.appendChild(inner);
							}
							// ensure a brand element exists
							if(!hdr.querySelector('.brand')){
								const brand = document.createElement('div'); brand.className='brand';
								const a = document.createElement('a'); a.href='/src/pages/Home/home.html';
								const img = document.createElement('img'); img.src='/src/assets/icons/img/logo.png'; img.className='brand-logo'; img.alt='logo';
								a.appendChild(img); brand.appendChild(a); inner.insertBefore(brand, inner.firstChild);
							}
						}
					}, 40);
